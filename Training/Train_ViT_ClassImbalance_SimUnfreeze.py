...
from ViTFinetune    import ViTSimultaneousUnfreeze
...
TRAINING_TYPE   = "ClassImbalance_Sim"
CHECKPOINT_ROOT = "../Checkpoints/"
os.makedirs(CHECKPOINT_ROOT, exist_ok=True)
CHECKPOINT_BASE = os.path.join(CHECKPOINT_ROOT, "vit_class_imbalance_sim/")
...
total_runs = len(STRATEGIES) * len(UNFREEZE_LAYERS) * len(EXPERIMENTS)
...
class EarlyStopping:
    def __init__(self, patience=5, save_path="best_model.pt", delta=1e-4):
    ...
    def __call__(self, val_loss, model):
        if val_loss < self.best_loss - self.delta:
            ...
            print(f"  checkmark New best val_loss={val_loss:.4f} - checkpoint saved.")
        else:
            self.counter += 1
...
def build_imbalanced_loaders(dataset_root, augs, image_size, batch_size,
    ...
    print(f"  Imbalanced train: {len(imbalanced_train)} samples (cat={n_cat}, dog={n_dog})")
...
def save_per_class_csv(per_class, checkpoint_dir, run_meta):
    """Long-format: one row per class index."""
    ...
    print(f"  Per-class CSV (long) saved to {csv_path}")
...
def save_training_curves(history, checkpoint_dir, title):
    ...
    plt.suptitle(f"ViT ClassImbalance SimUnfreeze - {title}", fontsize=11)
    plt.tight_layout()
...
def save_run_csv(results, checkpoint_dir):
    """Wide-format single-run CSV including all per-class columns."""
...
def save_master_csv(results, checkpoint_base):
    """
    Append to master CSV.
    Each row includes macro metrics AND p_c0..sup_c36 so the file is
    self-contained for cross-strategy, cross-experiment analysis.
    """
    csv_path    = os.path.join(checkpoint_base,
                               "all_experiment_results_sim_class_imbalance.csv")
...
if __name__ == "__main__":
    ...
    for strategy in STRATEGIES:
        ...
        for unfreeze_l in UNFREEZE_LAYERS:
            for AUGMENTATIONS in EXPERIMENTS:
                run_counter += 1
                aug_name    = AUGMENTATIONS["name"]
                run_label   = f"{strategy}_l{unfreeze_l}_{aug_name}"

                print("\n" + "=" * 70)
                print(f"RUN {run_counter}/{total_runs}  |  "
                      f"strategy={strategy}  layers={unfreeze_l}  aug={aug_name}")
                print("=" * 70)
                ...
                model = ViTSimultaneousUnfreeze(
                    num_classes=NUM_CLASSES, unfreeze_last_l=unfreeze_l, model_name=MODEL_NAME,
                )
                trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
                total     = sum(p.numel() for p in model.parameters())
                print(f"  Trainable: {trainable:,} / {total:,}  Frozen blocks: {model.frozen_block_count()}")
                ...
                print("\n-- Final Test Results --")
                print(f"  Strategy   : {strategy}  layers: {unfreeze_l}  aug: {aug_name}")
                ...
                save_training_curves(history, CHECKPOINT_DIR,
                                     title=f"{strategy} | L={unfreeze_l} | {aug_name}")
    ...
    print(f"Results CSV : {os.path.join(CHECKPOINT_BASE, 'all_experiment_results_sim_class_imbalance.csv')}")
