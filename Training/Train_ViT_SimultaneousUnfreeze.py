...
NUM_WORKERS     = 0
SEED            = 42
LEARNING_RATE   = 1e-4   # lower LR than linear probe — more params unfrozen
WEIGHT_DECAY    = 1e-4
LR_PATIENCE     = 3
LR_FACTOR       = 0.1
LR_MIN          = 1e-6
...
TRAINING_TYPE   = "Simultaneous_Unfreeze"
CHECKPOINT_ROOT = "../Checkpoints/"
os.makedirs(CHECKPOINT_ROOT, exist_ok=True)
CHECKPOINT_BASE = CHECKPOINT_ROOT + "vit_simultaneous_unfreeze/"
os.makedirs(CHECKPOINT_BASE, exist_ok=True)
NUM_CLASSES     = 37 if LABEL_MODE == "breed" else 2

# number of transformer blocks to unfreeze — each value becomes a separate run
...
print(f"Unfreeze layer configs: {UNFREEZE_LAYERS}")
print(f"Total runs: {len(EXPERIMENTS) * len(UNFREEZE_LAYERS)}")
...
def save_training_curves(history, checkpoint_dir, exp_name):
    ...
    plt.suptitle(f"ViT Simultaneous Unfreeze — {exp_name}", fontsize=13)
    plt.tight_layout()
...
# ─────────────────────────────────────────────────────────────────────────────
# EXPERIMENT LOOP — outer: unfreeze layers, inner: aug experiments
# ─────────────────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    total_runs = len(UNFREEZE_LAYERS) * len(EXPERIMENTS)
    run_idx    = 0
    ...
    for unfreeze_l in UNFREEZE_LAYERS:
        for exp_idx, AUGMENTATIONS in enumerate(EXPERIMENTS):
            ...
            run_name  = f"unfreeze{unfreeze_l}_{exp_name}"

            print("\n" + "=" * 70)
            print(f"RUN {run_idx}/{total_runs} — layers={unfreeze_l} | aug={exp_name}")
            print("=" * 70)

            # ── checkpoint dir — includes unfreeze_l in name ──────────────────
            CHECKPOINT_DIR = os.path.join(
                CHECKPOINT_BASE,
                f"{MODEL_NAME}_{TRAINING_TYPE}_l{unfreeze_l}_{LABEL_MODE}_{exp_name}"
            )
            ...
            model = ViTSimultaneousUnfreeze(
                ...
                unfreeze_last_l  = unfreeze_l,
                model_name       = MODEL_NAME,
            )

            trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
            total     = sum(p.numel() for p in model.parameters())
            print(f"Trainable : {trainable:>12,} / {total:,}  |  Frozen blocks: {model.frozen_block_count()}")
            ...
            save_training_curves(history, CHECKPOINT_DIR, run_name)
            save_results_csv({
